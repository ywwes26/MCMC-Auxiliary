...
function main() 
    ...
    @save "35_MALA_clip_stepsize2e-2_step10000.jld2" samples avg_accept_prob acc acc_time datause
...
function run_sampler(args::Dict, X::Array{Float64,2}, y::Array{Int64,1}, test_x::Array{Float64,2}, test_y::Array{Int64,1})
    sampler = MALA(X, y, args["stepsize"], args["pca_dim"], args["T"])
...
function mh_train(sampler::MALA, X::Array{Float64,2}, y::Array{Int64,1}, nsamples::Int64, stepsize::Float64, pca_dim::Int64, T::Float64, burnin::Int64, test_x::Array{Float64,2}, test_y::Array{Int64,1})
...
function MALA(X::Array{Float64,2}, y::Array{Int64,1}, stepsize::Float64, pca_dim::Int64, T::Float64)
    theta_prime = zeros(pca_dim)
    return MALA(X, y, stepsize, size(X, 2), pca_dim, T, theta_prime);
end

function next(self::MALA, theta::Array{Float64,1})
    data_size = self.data_size
    sig = 0
    theta_prime, proposal_mean, proposal_mean_prime = proposal(self, theta)
    logmh = 0.0
    for i = 1:data_size
...
function proposal(self::MALA, theta::Array{Float64,1})
    sigmoid_term = sigmoid.(self.X' * theta) .- self.y
    grad = self.X * sigmoid_term / self.T
    proposal_mean = theta - 0.5self.stepsize^2 * 2grad/norm(grad)
    #proposal_mean = theta - 0.5self.stepsize^2 * grad # not clipping
    ...
    proposal_mean_prime = self.theta_prime - 0.5self.stepsize^2 * 2grad_prime/norm(grad_prime)
    #proposal_mean_prime = self.theta_prime - 0.5self.stepsize^2 * grad_prime # not clipping
    return self.theta_prime, proposal_mean, proposal_mean_prime
end

function logp_proposal(self::MALA, theta::Array{Float64, 1}, prop_mean::Array{Float64, 1})
    return -0.5sum((theta .- prop_mean).^2)/(self.stepsize^2)
end

function stand_mh(self::MALA, u::Float64)
    return exp(u)
end
...
function logtarget(self::MALA,theta::Array{Float64,1},theta_prime::Array{Float64,1},idx::Int64)
    Xi_dot_theta = 0.0;
